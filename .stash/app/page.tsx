...
const SurfaceViewer = lazy(() => import('@/components/surface-viewer').then(m => ({ default: m.SurfaceViewer })))
...
export default function Home() {
  ...
  const handleReset = useCallback(async () => {
    ...
    try {
      ...
      if (response.success && response.mesh) {
        // Reset returns the original mesh
        setCurrentMesh(response.mesh)
      } else {
      ...
      <Suspense fallback={
        <div className="flex min-h-screen items-center justify-center bg-[#1a1a1a]">
          <div className="text-center">
            <div className="mb-4 size-12 animate-spin rounded-full border-4 border-white border-t-transparent" />
